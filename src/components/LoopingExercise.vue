      ...
      <div class="col-12">
        ...
        <div class="border p-1">
          ...
          <div class="blog" v-for="(value, key) in state.blog" :key="key">
            <p>{{ state.blog.title }}: {{ state.blog.description }} : {{ state.blog.author }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    setup() {
      ...
      return {
        ...
        addPlayer() {
          state.players.push({ name: state.name, number: state.number, position: state.position, photo: state.photo })
        }
